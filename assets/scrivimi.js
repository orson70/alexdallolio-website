...
(() => {
  ...
  .sv-b input,.sv-b textarea{width:100%;box-sizing:border-box;background:#000;border:1px solid rgba(236,235,231,.22);color:#ecebe7;font:15px/1.5 "Inter Tight",Helvetica,sans-serif;padding:10px 12px;border-radius:0}
  ...
  const p = document.createElement('div'); p.className = 'sv-p'; p.setAttribute('role', 'dialog'); p.setAttribute('aria-modal', 'true');
  p.innerHTML = `<form class="sv-b"><button type="button" class="sv-x" aria-label="${T.close}">×</button><button type="button" class="sv-bk">${T.back}</button>
    <h2>${T.h}</h2><p>${T.p}</p>
    <label for="sv-t">${T.msg}</label><textarea id="sv-t" name="pensiero" required></textarea>
  ...
  document.addEventListener('click', e => {
    ...
    if (!a) return;
    e.preventDefault(); p.classList.add('open'); setTimeout(() => p.querySelector('textarea').focus(), 50);
  });
  f.addEventListener('submit', async e => {
    e.preventDefault();
    const data = Object.fromEntries(new FormData(f).entries());
    if (data._honey) return;
    data.kind = 'note'; data.pensiero = `[${location.pathname}]\n${data.pensiero}`;
    btn.disabled = true; msg.textContent = T.sending;
    try {
      const r = await fetch(POSTINO, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
      ...
      btn.disabled = false;
      msg.innerHTML = `${T.err} <a href="mailto:alexdallolio@alexdallolio.com">alexdallolio@alexdallolio.com</a>`;
    }
  });
})();
